...
class Directorio:
    # Inicializamos el directorio cargando los datos de CSV
    def __init__(self, archivo="ContactosCSV.csv"):
        ...
        self.archivo = archivo # Creamos una copia del archivo para manejarlo 
        self.Importar_Datos()# Funcion para importar datos de CSV

    # Funcion para importar datos de CSV
    def Importar_Datos(self):
        # Importamos los contactos de CSV
        try:
            ...
            with open(self.archivo, mode='r') as file:
                ...
                for row in reader:
                    if len(row) == 6:
                        self.agregar_contacto(row[0], row[1], row[2], row[3], row[4], row[5])
        except FileNotFoundError:
            pass
    
    # Funcion para exportar los datos nuevos a CSV
    def Exportar_Datos(self):
        # Abrimos el archivo CSV en modo escritura
        with open(self.archivo, mode='w', newline='') as file:
            writer = csv.writer(file)
            # Identificamos las columnas
            writer.writerow(["id", "Nombre", "Telefono", "Fecha de Nacimiento", "Correo", "Área"])
            for contacto in self.contactos:
                writer.writerow([contacto.id, contacto.nombre, contacto.telefono, contacto.fecha_nacimiento, contacto.correo, contacto.area])
    
    # Funcion para agregar los contactos al local y archivo CSV
    def agregar_contacto(self, id, nombre, telefono, fecha_nacimiento, correo, area):
        ...
        # Subimos el contacto al archivo CSV
        self.Exportar_Datos()
